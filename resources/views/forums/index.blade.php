    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        @forelse($forums as $forum)
            <a href="{{ route('forums.show', $forum) }}"
               class="group p-6 rounded-lg bg-gruvbox-light-bg1 dark:bg-gruvbox-dark-bg1 border-2 border-gruvbox-light-bg3 dark:border-gruvbox-dark-bg3 hover:border-gruvbox-light-blue dark:hover:border-gruvbox-dark-blue transition-all hover:shadow-lg">
                <div class="flex items-start justify-between mb-3">
                    <h3 class="text-xl font-bold text-gruvbox-light-fg0 dark:text-gruvbox-dark-fg0 group-hover:text-gruvbox-light-blue dark:group-hover:text-gruvbox-dark-blue transition-colors">{{ $forum->name }}</h3>
                    ...
                    <div>
                        @if($forum->latestPost)
                            <div class="flex items-center text-sm text-gruvbox-light-fg3 dark:text-gruvbox-dark-fg3">
                                ...
                                Last post {{ $forum->latestPost->created_at->diffForHumans() }}
                            </div>
                        @else
                            <p class="text-sm text-gruvbox-light-aqua dark:text-gruvbox-dark-aqua font-semibold">Be the first to post!</p>
                        @endif
                    </div>
                    <div class="flex items-center text-sm font-semibold text-gruvbox-light-blue dark:text-gruvbox-dark-blue group-hover:translate-x-1 transition-transform">
                        Enter
