<x-app-layout>
        ...
        <div class="flex justify-between items-center">
            <div>
                <h2 class="font-semibold text-2xl text-gruvbox-light-fg0 dark:text-gruvbox-dark-fg0">
                    {{ $studyGroup->name }} - Calendar
                </h2>
            </div>
            @if($isModerator)
                <button onclick="document.getElementById('createEventModal').classList.remove('hidden')" class="px-4 py-2 bg-gruvbox-light-blue dark:bg-gruvbox-dark-blue text-gruvbox-light-bg0 dark:text-gruvbox-dark-bg0 rounded-lg hover:opacity-90 transition-opacity font-medium">
                    Add Event
                </button>
            @endif
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="p-6 rounded-lg bg-gruvbox-light-bg1 dark:bg-gruvbox-dark-bg1 border border-gruvbox-light-bg3 dark:border-gruvbox-dark-bg3">
            ...
            @forelse($events->where('is_completed', false) as $event)
                <div class="mb-4 p-4 rounded-lg bg-gruvbox-light-bg2 dark:bg-gruvbox-dark-bg2 border border-gruvbox-light-bg3 dark:border-gruvbox-dark-bg3">
                    ...
                    <div class="flex items-center text-sm text-gruvbox-light-fg3 dark:text-gruvbox-dark-fg3 mb-2">
                    ...
                    <div class="flex gap-2 flex-wrap">
                        @if($isModerator)
                            ...
                            <form action="{{ route('study-groups.calendar.destroy', $event) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this event?')">
                                @csrf
                                @method('DELETE')
            ...
            @empty
                <p class="text-gruvbox-light-fg3 dark:text-gruvbox-dark-fg3 text-center py-8">No upcoming events</p>
            @endforelse
        </div>

        <!-- Completed Events -->
        <div class="p-6 rounded-lg bg-gruvbox-light-bg1 dark:bg-gruvbox-dark-bg1 border border-gruvbox-light-bg3 dark:border-gruvbox-dark-bg3">
            <h3 class="text-xl font-bold text-gruvbox-light-fg0 dark:text-gruvbox-dark-fg0 mb-4">Completed Events</h3>

            @forelse($events->where('is_completed', true) as $event)
    ...
    @if($isModerator)
        <div id="createEventModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-gruvbox-light-bg0 dark:bg-gruvbox-dark-bg0 rounded-lg p-6 max-w-md w-full mx-4">
                <h3 class="text-xl font-bold text-gruvbox-light-fg0 dark:text-gruvbox-dark-fg0 mb-4">Add New Event</h3>

                <form action="{{ route('study-groups.calendar.store', $studyGroup) }}" method="POST">
                    @csrf
                    <div class="space-y-4">
                        <div>
        ...
        <div id="editEventModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-gruvbox-light-bg0 dark:bg-gruvbox-dark-bg0 rounded-lg p-6 max-w-md w-full mx-4">
                <h3 class="text-xl font-bold text-gruvbox-light-fg0 dark:text-gruvbox-dark-fg0 mb-4">Edit Event</h3>
        ...
        <script>
            function openEditModal(event) {
                document.getElementById('editEventForm').action = `/study-groups/calendar/${event.id}`;
